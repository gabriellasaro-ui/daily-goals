...
// Usamos essa constante para renderizar os botões de edição também
...
interface MissionCardProps {
  ...
  onEdit: (id: string, newTitle: string, newCategory: string) => void; // Nova prop
}
...
export const MissionCard = ({ mission, onToggle, onDelete, onEdit }: MissionCardProps) => {
  ...
  const handleToggle = () => {
    if (isEditing) return; // Não completa se estiver editando
    setIsAnimating(true);
    setTimeout(() => {
      onToggle(mission.id);
  ...
  const handleSave = () => {
    if (editedTitle.trim()) {
      onEdit(mission.id, editedTitle, editedCategory);
      setIsEditing(false);
    }
  };
  ...
  const handleCancel = () => {
    ...
    setEditedCategory(mission.category || "outros");
    setIsEditing(false);
  };
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
        ...
        {/* Se estiver editando, esconde o botão de check */}
        {!isEditing && (
          <Button
            variant="ghost"
            size="icon"
        ...
        {/* CONTEÚDO PRINCIPAL (Muda se for edição ou visualização) */}
        ...
        <div className="flex items-center gap-1">
          {isEditing ? (
            <>
              <Button size="icon" variant="ghost" onClick={handleSave} className="text-green-600 hover:text-green-700 hover:bg-green-100">
                <Save className="h-5 w-5" />
              </Button>
              <Button size="icon" variant="ghost" onClick={handleCancel} className="text-gray-400 hover:text-gray-600 hover:bg-gray-100">
                <X className="h-5 w-5" />
              </Button>
            </>
          ) : (
